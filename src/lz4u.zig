...
pub const Decompress = @import("lz4u/Decompress.zig");
pub const Compress = @import("lz4u/Compress.zig");

pub const max_history_len = 64 * 1024;
pub const max_window_len = max_history_len * 2;
pub const max_block_size = BlockMaxSize.@"4MB".toBytes();
...
fn testDecompressIndirect(gpa: mem.Allocator, compressed: []const u8) ![]u8 {
    const indirect_buf = try gpa.alloc(u8, min_indirect_buffer_len);
...
fn testDiscard(gpa: mem.Allocator, compressed: []const u8) !usize {
    const buf = try gpa.alloc(u8, min_indirect_buffer_len);
    defer gpa.free(buf);
...
fn testCompress(gpa: mem.Allocator, uncompressed: []const u8, options: Compress.Options) ![]u8 {
    ...
    const window_buf = try gpa.alloc(u8, max_window_len);

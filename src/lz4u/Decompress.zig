...
pub fn readFrameHeader(reader: *Reader) ReadHeaderError!Frame {
    ...
    const flg = try reader.takeStruct(Frame.Flg, .little);
    if (flg.version != 1) {
        return error.UnknownVersion;
    }
    hash.autoHash(&checksum, @as(u8, @bitCast(flg)));

    const bd = try reader.takeStruct(Frame.Bd, .little);
